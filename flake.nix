    ...
    in
    {
      packages.${system} =
        {
          ...
          beamer-present = pkgs.writeShellApplication {
            ...
            text = ''
              ...
              pdfpc -d 15 -l 3 -W -g -R ./beamer/out/Beamer.pdfpc --note-format=markdown ./beamer/out/Beamer.pdf

              echo "Starting presentation with pdfpc..."
              sleep 1
              hyprctl dispatch workspace 12 || true # focus empty workspace
              hyprctl dispatch workspace 5 || true  # focus empty workspace
              sleep 1

              (sleep 1; hyprctl dispatch focusmonitor eDP-1; echo "focus internal monitor")&
              
              echo "Presentation started. Use Ctrl+C to stop."
              pdfpc -d 15 -l 3 -W -g -R ./beamer/out/Beamer.pdfpc --note-format=markdown --page-transition "fade:0.3" ./beamer/out/Beamer.pdf
            '';
          };
      };
      ...
      devShells.${system}.default = pkgs.mkShell {
        buildInputs = [
          ...
          # idk why but pdfpc(4.7.0) from nix doesnt render the notes
          pkgs-old.pdfpc
        ];
      };
    };
